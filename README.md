...
**If you set up your Sheet before receipts were added:** just redeploy
a new version as above — the script now adds any missing columns
(`paidDate`, `receiptNumber`) to your existing "Invoices" tab
automatically the next time it runs, without touching your existing
rows.
...
## PDF export
Uses `html2canvas` + `jsPDF` (loaded from cdnjs) to capture the invoice
sheet and place it on a single PDF page sized to fit exactly what's
rendered — however long the item list gets, nothing is cut off.

## Receipts
Once an invoice's status is set to **Paid** (via Edit, or when creating
it), open that invoice and click **Generate Receipt**. The first click
assigns it a receipt number (`RCPT-{year}-{seq}`) and saves it back to
the Sheet; after that, the same button toggles between the Invoice and
Receipt views instantly. Both Print and Download PDF act on whichever
view is currently open. The receipt number is remembered, so re-opening
an invoice later and generating its receipt again reuses the same
number rather than creating a new one.
...
The app is installable — Niko or Ginger can add it to their home
screen and it opens full-screen, like a native app, with its own icon.

- **Android (Chrome):** open the site → tap the **⋮** menu → **Add to
  Home screen** / **Install app**. Chrome may also prompt this
  automatically.
- **iPhone (Safari):** open the site → tap the **Share** icon → **Add
  to Home Screen**.

This works through a standard web app manifest (`manifest.json`,
`assets/icons/`) plus a small service worker (`sw.js`) that caches
only the app's own files (HTML/CSS/JS/logo) so it opens instantly.
It deliberately never caches the Google Sheets API — invoice data
always comes fresh from the network, so nothing goes stale.
