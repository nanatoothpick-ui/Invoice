...
const COMPANY = {
  ...
  address: "Hansan Road, Timber Market,\nAccra, Ghana.",
  paymentAccountName: "Taylor Emmanuel Kobina",
...
let activeDocType = "invoice"; // "invoice" | "receipt" — which view is currently open
...
function nextReceiptNumberFromCache(){
  const year = new Date().getFullYear();
...
// Only relevant once an invoice is marked Paid — shows the "Date paid"
// field and defaults it to today, so a receipt can reference it later.
function togglePaidDateField(){
  const isPaid = $("#f-status").value === "paid";
  $("#paiddate-field").hidden = !isPaid;
  if (isPaid && !$("#f-paiddate").value){
    $("#f-paiddate").value = new Date().toISOString().slice(0,10);
  }
}
$("#f-status").addEventListener("change", togglePaidDateField);
...
function collectFormData(){
  ...
  return {
    ...
    date: $("#f-date").value,
    status,
    paidDate: status === "paid" ? ($("#f-paiddate").value || new Date().toISOString().slice(0,10)) : "",
...
// Redraws whichever document (invoice or receipt) is currently active
// for the open record, and keeps the action buttons in sync with it.
function refreshDetailView(){
  const inv = invoicesCache.find(i => i.id === activeDetailId);
  if (!inv) return;
  renderInvoiceSheet(inv, activeDocType);
  $("#detail-heading").textContent = activeDocType === "receipt"
    ? `Receipt · ${inv.receiptNumber || "—"}`
    : inv.number;
  const receiptBtn = $("#detail-receipt-btn");
  receiptBtn.disabled = inv.status !== "paid";
  receiptBtn.title = inv.status !== "paid" ? "Mark this invoice as Paid to generate a receipt" : "";
  receiptBtn.textContent = activeDocType === "receipt" ? "View Invoice" : "View Receipt";
}

function toggleDetailActionsForPreview(isPreview){
  $("#detail-edit-btn").hidden = isPreview;
  $("#detail-duplicate-btn").hidden = isPreview;
  $("#detail-receipt-btn").hidden = isPreview;
...
function renderInvoiceSheet(inv, docType){
  ...
  const metaRows = isReceipt ? `
    <tr><td>Receipt #:</td><td>${escapeHtml(inv.receiptNumber || "—")}</td></tr>
    <tr><td>For Invoice #:</td><td>${escapeHtml(inv.number)}</td></tr>
    <tr><td>Date paid:</td><td>${formatDate(inv.paidDate || inv.date)}</td></tr>
  ` : `
    <tr><td>Invoice #:</td><td>${escapeHtml(inv.number)}</td></tr>
    <tr><td>Date:</td><td>${formatDate(inv.date)}</td></tr>
    <tr><td>Status:</td><td><span class="status-badge status-${inv.status}">${inv.status}</span></td></tr>
  `;

  const summarySection = isReceipt ? `
    <div class="inv-summary-row">
      <div class="inv-payment-info">
        <h3>Payment received</h3>
        <p>Payment for Invoice ${escapeHtml(inv.number)} has been received in full via ${escapeHtml(COMPANY.paymentType)}.</p>
        <p>Thank you for your business.</p>
      </div>
      <div class="inv-total-box">
        <div class="inv-total-line inv-total-grand">
          <span>Amount Paid</span><strong>${money(subtotal)}</strong>
        </div>
      </div>
    </div>
  ` : `
    <div class="inv-summary-row">
      <div class="inv-payment-info">
        ...
        <p>Please include Invoice Number <strong>${escapeHtml(inv.number)}</strong> as the payment reference.</p>
        <p>Account Name: ${escapeHtml(COMPANY.paymentAccountName)}</p>
        <p>Account Number: ${escapeHtml(COMPANY.paymentNumber)}</p>
        <p>Payment Type: ${escapeHtml(COMPANY.paymentType)}</p>
      </div>
      <div class="inv-total-box">
        <div class="inv-total-line inv-total-grand">
          <span>Total Due</span><strong>${money(subtotal)}</strong>
        </div>
      </div>
    </div>
  `;
  ...
  $("#invoice-print-area").innerHTML = `
    ${isReceipt ? '<div class="inv-watermark">PAID</div>' : ""}
    <div class="inv-header-row">
      ...
      <div class="inv-doc-meta">
        <div class="inv-doc-title">${isReceipt ? "Receipt" : "Invoice"}</div>
        <table class="inv-meta-table">${metaRows}</table>
      </div>
    </div>
    ...
    <div class="inv-bill-row">
      <div class="inv-box inv-bill-to">
        ...
        <p>${inv.clientContact ? escapeHtml(inv.clientContact) : "—"}</p>
      </div>
      <div class="inv-box inv-remit">
        <h3>Remit to / from</h3>
        <strong>${COMPANY.name}</strong>
...
// A small buffer is added since fonts/measurement can be a touch off.
function getFitPageSizeMM(el){
  const heightPx = el.scrollHeight;
  const heightMm = pxToMm(heightPx) + PAGE_MARGIN_MM * 2 + 6;
  return [SHEET_WIDTH_MM, Math.max(heightMm, 148)]; // never smaller than A5 height
}

// Wait two animation frames so layout (fonts, the logo image, etc.) has
// fully settled before we measure — avoids sizing the page off a height
// that's still mid-reflow.
function nextFrame(){
  return new Promise(resolve => requestAnimationFrame(() => requestAnimationFrame(resolve)));
}

$("#detail-print-btn").addEventListener("click", async () => {
...
$("#detail-pdf-btn").addEventListener("click", async () => {
  const inv = invoicesCache.find(i => i.id === activeDetailId);
  const baseName = activeDocType === "receipt" ? (inv && inv.receiptNumber) : (inv && inv.number);
  const filename = (baseName || (activeDocType === "receipt" ? "receipt" : "invoice"))
    + (activeDocType === "receipt" ? "-receipt" : "") + ".pdf";
  const el = $("#invoice-print-area");
  const btn = $("#detail-pdf-btn");
  ...
  try {
    await nextFrame();
    const scale = 2;
    // windowHeight/scrollY here matter: without them html2canvas only
    // captures the part of the element currently in the viewport, which
    // is what was silently cropping the bottom off longer invoices.
    ...
    const widthMm = pxToMm(canvas.width / scale);
    const heightMm = pxToMm(canvas.height / scale);
    const { jsPDF } = window.jspdf;
    const pdf = new jsPDF({
      unit: "mm",
      format: [widthMm + PAGE_MARGIN_MM * 2, heightMm + PAGE_MARGIN_MM * 2],
      orientation: "portrait"
    });
    pdf.addImage(canvas.toDataURL("image/jpeg", 0.98), "JPEG", PAGE_MARGIN_MM, PAGE_MARGIN_MM, widthMm, heightMm);
    pdf.save(filename);
  } catch (err){
    showError("Couldn't generate PDF: " + err.message);
  } finally {
    btn.disabled = false;
...
$("#detail-edit-btn").addEventListener("click", () => {
  ...
  $("#f-date").value = inv.date || new Date().toISOString().slice(0,10);
  $("#f-status").value = inv.status || "pending";
  $("#f-paiddate").value = inv.paidDate || "";
  $("#f-notes").value = inv.notes || "";
  togglePaidDateField();
  $("#invoice-number-tag").textContent = inv.number;
  renderItems();
  updateTotals();
  $("#nav-new").classList.add("is-active");
  $("#nav-history").classList.remove("is-active");
  pageDetail.hidden = true; pageNew.hidden = false;
});

$("#detail-receipt-btn").addEventListener("click", async () => {
  const inv = invoicesCache.find(i => i.id === activeDetailId);
  if (!inv) return;

  if (activeDocType === "receipt"){
    activeDocType = "invoice";
    refreshDetailView();
    return;
  }
  if (inv.status !== "paid") return; // guarded by disabled state too

  const btn = $("#detail-receipt-btn");
  btn.disabled = true;
  btn.textContent = "Preparing…";
  try {
    if (!inv.receiptNumber){
      const receiptNumber = nextReceiptNumberFromCache();
      const updated = Object.assign({}, inv, { receiptNumber });
      await apiPost({ action: "save", invoice: updated });
      // Update the cache in place with what we know is correct, rather
      // than re-fetching from the Sheet — an Apps Script deployment that
      // hasn't been redeployed since receipts were added would silently
      // drop the new column, and a re-fetch would wipe this number right
      // back out.
      const idx = invoicesCache.findIndex(i => i.id === inv.id);
      if (idx !== -1) invoicesCache[idx] = updated;
    }
    activeDocType = "receipt";
    refreshDetailView();
  } catch (err){
    showError("Couldn't generate receipt: " + err.message);
    refreshDetailView(); // restore correct button state/label
  }
});
